...
def plot_bboxes(image, bboxes, line_thickness=None):
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        if pos_id < len(color_list):
            color = color_list[pos_id-1]
        else:
    ...
    cv2.putText(image, 'Count-{}'.format(max(max_id,count)), (50,50), cv2.FONT_HERSHEY_SIMPLEX,1, (255, 255, 255), 2, cv2.LINE_AA)
    count = max(max_id,count)
    return image
