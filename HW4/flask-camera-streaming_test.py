...
def check_position_bbox(position, bbox):
    x, y = position
    x1, y1, x2, y2 = bbox
...
def disable_object(x,y):
    for t in tracker.trackers:
        if check_position_bbox((x,y), t.get_state()[0]):
            if(t.is_show): t.is_show= False
            else: t.is_show= True
